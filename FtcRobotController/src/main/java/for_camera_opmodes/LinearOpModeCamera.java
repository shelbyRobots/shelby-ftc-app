 ...
 */
@SuppressWarnings({"deprecation", "WeakerAccess", "FieldCanBeLocal", "UnusedAssignment", "unused"})
public class LinearOpModeCamera extends LinearOpMode {
  ...
  public Camera.PreviewCallback previewCallback = new Camera.PreviewCallback()
  {
    public void onPreviewFrame(byte[] data, Camera camera)
    {
      ...
      {
        DbgLog.error("Exception in PreviewCallback");
      }
    }
  };
  ...
  public Camera initCamera(int cameraInfoType)
  {
    int cameraId = -1;
    ...
    String data = parameters.flatten();

    return camera;
  }

  public void startCamera(int cameraInfoType)
  {
    ...
    if (preview == null)
    {
      ((FtcRobotControllerActivity) hardwareMap.appContext)
              .initPreviewLinear(camera, this, previewCallback);
    }
  }
  ...
  public class CameraKillThread implements Runnable
  {
    ...
    public void run()
    {
      ...
      {
        DbgLog.error("CameraKillThread interupted");
      }
  ...
  static public int red(int pixel)   { return (pixel >> 16) & 0xff; }
  static public int green(int pixel) {
    return (pixel >>  8) & 0xff;
  }
  static public int blue(int pixel)  { return pixel & 0xff; }
  ...
  static public Bitmap convertYuvImageToRgb(YuvImage yuvImage,
